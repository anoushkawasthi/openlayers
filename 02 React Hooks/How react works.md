...
Phase 1: Trigger  
A render is triggered either by the initial loading or a state change. React adds it to the Queue.
...
1. React FIber:   Before React 16 the Reconciliation was synchronus. If there was a massive component DOM tree it would lock up the main thread while comparing the old and new VDOMs's, which would cause the browser to drop frames.   React rewrote its core engine   
-> React Fiber     
A Linked List: Instead of a strict tree, Fiber turns the VDOM into a linked list of "Fiber nodes." Each node represents a component and keeps track of its own state, props, and DOM element. Interruptible Work: Because it's a linked list, React can pause its rendering work, yield control back to the browser so it can handle user inputs or animations, and then resume rendering exactly where it left off. This is called Time Slicing or Concurrent Rendering
